...
class Server:
    ...
    def sendMoveMessage(self, AGV_id):
        """ Creates a connection with the selected AGV and reads the latest state it published """
        ...
        while not sent:
            try:
                print("[BT]: Sending data.")
                self.device.char_write("0000ffe1-0000-1000-8000-00805f9b34fb", msg)
                print("[BT] Sent data:  %s" % (msg.decode("utf-8")))
                sent = True
            except:
                print("[BT]: Unable to send, trying again.")
